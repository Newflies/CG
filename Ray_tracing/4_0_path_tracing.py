...
import numpy as np
import argparse
from ray_tracing_models import Triangle, Ray, Camera, Hittable_list, Sphere, PI, random_in_unit_sphere, refract, reflect, reflectance, random_unit_vector
ti.init(arch=ti.gpu)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--max_depth', type=int, default=5, help='max depth (default: 10)')
    parser.add_argument(
    ...
    scene.add(Triangle(a=ti.Vector([0,-0,-1]),b=ti.Vector([-6,2,-1]),c=ti.Vector([-10,-2,-1]),material=1, color=ti.Vector([0.0, 1.0, 1.0])))
